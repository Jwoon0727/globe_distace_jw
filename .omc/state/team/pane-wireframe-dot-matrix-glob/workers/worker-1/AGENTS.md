...
```bash
mkdir -p $(dirname .omc/state/team/pane-wireframe-dot-matrix-glob/workers/worker-1/.ready) && touch .omc/state/team/pane-wireframe-dot-matrix-glob/workers/worker-1/.ready
```
...
1. **Claim** your task (run this command first):
   `omc team api claim-task --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\",\"worker\":\"worker-1\"}" --json`
...
3. **Send ACK** to the leader:
   `omc team api send-message --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"from_worker\":\"worker-1\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: worker-1 initialized\"}" --json`
4. **Transition** the task status (REQUIRED before exit):
   - On success: `omc team api transition-task-status --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\",\"from\":\"in_progress\",\"to\":\"completed\",\"claim_token\":\"<claim_token>\",\"result\":\"Summary: <what changed>\\nVerification: <tests/checks run>\\nSubagent skip reason: worker protocol forbids nested subagents; completed focused probe in-session\"}" --json`
   - On failure: `omc team api transition-task-status --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\",\"from\":\"in_progress\",\"to\":\"failed\",\"claim_token\":\"<claim_token>\"}" --json`
...
## Identity
- **Team**: pane-wireframe-dot-matrix-glob
- **Worker**: worker-1
- **Agent Type**: claude
- **Environment**: OMC_TEAM_WORKER=pane-wireframe-dot-matrix-glob/worker-1

## Your Tasks
- **Task 1**: 구현 전담 워커. 메인 pane(리더)이 전달하는 구현 계획/플랜에 따라 기능을 구현·수정한다. 플랜을 받기 전까지는 대기하고, 리더가 태스크/
  Description: 구현 전담 워커. 메인 pane(리더)이 전달하는 구현 계획/플랜에 따라 기능을 구현·수정한다. 플랜을 받기 전까지는 대기하고, 리더가 태스크/메시지로 전달하는 범위만 구현한다. 프로젝트: wireframe-dot-matrix-globe (Next.js, pnpm). 빌드=pnpm build, 린트=pnpm lint.
  Status: pending

## Task Lifecycle Reference (CLI API)
Use the CLI API for all task lifecycle operations. Do NOT directly edit task files.

- Inspect task state: `omc team api read-task --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\"}" --json`
- Task id format: State/CLI APIs use task_id: "<id>" (example: "1"), not "task-1"
- Claim task: `omc team api claim-task --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\",\"worker\":\"worker-1\"}" --json`
- Complete task: `omc team api transition-task-status --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\",\"from\":\"in_progress\",\"to\":\"completed\",\"claim_token\":\"<claim_token>\",\"result\":\"Summary: <what changed>\\nVerification: <tests/checks run>\\nSubagent skip reason: worker protocol forbids nested subagents; completed focused probe in-session\"}" --json`
- Fail task: `omc team api transition-task-status --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\",\"from\":\"in_progress\",\"to\":\"failed\",\"claim_token\":\"<claim_token>\"}" --json`
- Release claim (rollback): `omc team api release-task-claim --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"task_id\":\"<id>\",\"claim_token\":\"<claim_token>\",\"worker\":\"worker-1\"}" --json`
...
- Resolve the team state root in this order: `OMC_TEAM_STATE_ROOT` env -> worker identity `team_state_root` -> config/manifest `team_state_root` -> /Users/jwoonsm5/Desktop/MacM5Dev/distance_globe/wireframe-dot-matrix-globe/.omc/state/team/pane-wireframe-dot-matrix-glob.
- `OMC_TEAM_STATE_ROOT` is the team-specific root (`.../.omc/state/team/pane-wireframe-dot-matrix-glob`). When it is set, append worker/mailbox paths directly below it; do not append another `team/pane-wireframe-dot-matrix-glob` segment.
...
- **Inbox**: Read .omc/state/team/pane-wireframe-dot-matrix-glob/workers/worker-1/inbox.md for new instructions
- **Status**: Write to .omc/state/team/pane-wireframe-dot-matrix-glob/workers/worker-1/status.json:
  ```json
  {"state": "idle", "updated_at": "<ISO timestamp>"}
  ```
  States: "idle" | "working" | "blocked" | "done" | "failed"
- **Heartbeat**: Update .omc/state/team/pane-wireframe-dot-matrix-glob/workers/worker-1/heartbeat.json every few minutes:
  ```json
...
- To leader: `omc team api send-message --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"from_worker\":\"worker-1\",\"to_worker\":\"leader-fixed\",\"body\":\"<message>\"}" --json`
- Check mailbox: `omc team api mailbox-list --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"worker\":\"worker-1\"}" --json`
- Mark delivered: `omc team api mailbox-mark-delivered --input "{\"team_name\":\"pane-wireframe-dot-matrix-glob\",\"worker\":\"worker-1\",\"message_id\":\"<id>\"}" --json`
...
1. Write your decision to: .omc/state/team/pane-wireframe-dot-matrix-glob/workers/worker-1/shutdown-ack.json
2. Format:
